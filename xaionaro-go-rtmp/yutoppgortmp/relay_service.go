...
type RelayService struct {
	...
	streams        map[AppKey]*Pubsub
	streamsChanged chan struct{}
}

func NewRelayService() *RelayService {
	return &RelayService{
		streams:        make(map[AppKey]*Pubsub),
		streamsChanged: make(chan struct{}),
	}
}

func (s *RelayService) NewPubsub(key AppKey, publisherHandler *Handler) (*Pubsub, error) {
	ctx := context.TODO()
...
func (s *RelayService) GetPubsub(key AppKey) *Pubsub {
	ctx := context.TODO()
...
func (s *RelayService) Pubsubs() map[AppKey]*Pubsub {
	ctx := context.TODO()
	return xsync.DoR1(ctx, &s.m, func() map[AppKey]*Pubsub {
		m := make(map[AppKey]*Pubsub, len(s.streams))
...
func (s *RelayService) PubsubNames() []AppKey {
	ctx := context.TODO()
	return xsync.DoR1(ctx, &s.m, func() []AppKey {
		result := make([]AppKey, 0, len(s.streams))
...
func (s *RelayService) RemovePubsub(key AppKey) error {
	ctx := context.TODO()
